...
class TSP(Problem):
    ...
    def generate_params(self, mode: str = "train", list_coords=None) -> Dict[str, torch.Tensor]:
        """Generate coordinates for TSP instances.

        If `list_coords` is provided, sample each instance from that pool; otherwise
        ...
        pool = torch.tensor(list_coords, dtype=torch.float32, device=self.device)  # [N, 2]
        N = pool.shape[0]
        assert self.dim <= N, "dim no puede exceder el tamaño del pool si es sin reemplazo"

        # Cada instancia elige 'dim' ciudades distintas del pool
        coords = torch.stack([
            pool[torch.randperm(N, generator=self.generator)[:self.dim]]
            for _ in range(self.n_problems)
        ], dim=0)  # [n_problems, dim, 2]
        return {"coords": coords}

        # pacientes_df = pd.read_excel("pacientes_small.xlsx")
        # # Divide la columna "COORDENADAS" en latitud y longitud y agrégalas como nuevas columnas al DataFrame
        ...
        #     self.manual_seed(0)
        #     # INSERT_YOUR_CODE
        #     # Selecciona solo las primeras self.dim filas y las columnas normalizadas
        #     coords_np = pacientes_df[["lat_normalizado", "lon_normalizado"]].values[:self.dim]
        #     # Expande a shape (self.n_problems, self.dim, 2)
        #     coords = torch.tensor(coords_np, dtype=torch.float32, device=self.device)
        #     coords = coords.unsqueeze(0).expand(self.n_problems, -1, -1).clone()
        # return {"coords": coords}
        # if mode == "test":
        #     self.manual_seed(0)
        # coords = torch.rand(
